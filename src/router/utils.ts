    ...
    if (asyncRouteList && asyncRouteList?.length > 0) {
      ...
      return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
          console.error("获取动态路由超时");
          storageLocal().removeItem(key);
          reject(new Error("获取动态路由超时"));
        }, timeout);
...
function formatFlatteningRoutes(routesList: RouteRecordRaw[]) {
  ...
  for (let i = 0; i < hierarchyList.length; i++) {
    ...
    if (iterations > maxIterations) {
      console.warn(
        "[路由警告] formatFlatteningRoutes 超过最大迭代次数，可能存在循环引用"
      );
      break;
    }
...
function addAsyncRoutes(arrRoutes: Array<RouteRecordRaw>) {
  ...
  // 使用 filter 过滤掉找不到组件的路由
  return arrRoutes.filter((v: RouteRecordRaw) => {
    ...
    if (v.meta?.frameSrc) {
      ...
      // 如果找不到对应的组件文件，跳过该路由（不添加到路由表）
      if (index === -1) {
        console.warn(
          `[路由警告] 找不到组件文件: ${v.component || v.path}，已跳过该路由。请先创建对应的 Vue 组件文件。`
        );
        return false; // 过滤掉该路由
      }
